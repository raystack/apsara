---
title: Link
description: Display a hyperlink component with various styles and behaviors
---

## Preview

<Preview>
  <Flex style={{ gap: "24px", flexWrap: "wrap" }}>
    <Link href="#" variant="primary">Primary Link</Link>
    <Link href="#" variant="accent">Accent Link</Link>
    <Link href="#" underline>Underlined Link</Link>
    <Link href="https://google.com" external>External Link</Link>
    <Link href="#" variant="accent" size={6}>Large Link</Link>
  </Flex>
</Preview>

## Usage

The Link component provides a consistent way to navigate between pages or sections. It supports various styles, sizes, and behaviors including external links and underlined variants.

## Installation

Install the component from your command line.

<LiveProvider>
  <LiveEditor code={`npm install @raystack/apsara`} border language="shell" />
</LiveProvider>

<LiveProvider>
  <LiveEditor code={`import { Link } from '@raystack/apsara/v1'

<Link href="#" variant="accent">
  Accent Link
</Link>`} border/>
</LiveProvider>

## Link Props

The Link component accepts the following props:

- `href`: The URL that the link points to (required)
- `variant`: Visual style variant (`"primary"` | `"secondary"` | `"tertiary"` | `"emphasis"` | `"accent"` | `"attention"` | `"danger"` | `"success"`, default: "accent")
- `size`: Text size (1-10, default: 2)
- `weight`: Font weight (`"normal"` | `"bold"` | `"bolder"` | number, default: 400)
- `underline`: Boolean to show underline decoration
- `external`: Boolean to open link in new tab
- `download`: Boolean or string for downloadable links
- `className`: Additional CSS class names

## Variants

Choose from different variants to match your design needs and content hierarchy.

<Playground
  scope={{ Link }}
  tabs={[
    {
      name: "Primary",
      code: `
      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        <Link href="#" variant="primary">Primary Link</Link>
        <Link href="#" variant="secondary">Secondary Link</Link>
        <Link href="#" variant="tertiary">Tertiary Link</Link>
        <Link href="#" variant="accent">Accent Link</Link>
        <Link href="#" variant="attention">Attention Link</Link>
        <Link href="#" variant="danger">Danger Link</Link>
        <Link href="#" variant="success">Success Link</Link>
      </div>`,
    },
  ]}
/>

## Sizes

The Link component supports 10 different sizes to accommodate various layout requirements.

<Playground
  scope={{ Link }}
  tabs={[
    {
      name: "Sizes",
      code: `
      <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-end', flexWrap: 'wrap' }}>
        <Link href="#" size={1}>Size 1</Link>
        <Link href="#" size={2}>Size 2</Link>
        <Link href="#" size={3}>Size 3</Link>
        <Link href="#" size={4}>Size 4</Link>
        <Link href="#" size={5}>Size 5</Link>
        <Link href="#" size={6}>Size 6</Link>
        <Link href="#" size={7}>Size 7</Link>
        <Link href="#" size={8}>Size 8</Link>
        <Link href="#" size={9}>Size 9</Link>
        <Link href="#" size={10}>Size 10</Link>
      </div>`,
    },
  ]}
/>

## Weights

Control the visual emphasis of links with different font weights.

<Playground
  scope={{ Link }}
  tabs={[
    {
      name: "Weights",
      code: `
      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        <Link href="#" weight="normal">Normal Weight</Link>
        <Link href="#" weight="bold">Bold Weight</Link>
        <Link href="#" weight={500}>Weight 500</Link>
      </div>`,
    },
  ]}
/>

## Styles

Additional style variations including underlined and external links.

<Playground
  scope={{ Link }}
  tabs={[
    {
      name: "Underline",
      code: `
      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        <Link href="#" variant="primary" underline>Underlined Link</Link>
        <Link href="#" variant="accent" underline>Underlined Accent</Link>
      </div>`,
    },
    {
      name: "External",
      code: `
      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        <Link href="https://example.com" external>External Link</Link>
        <Link href="https://example.com" external variant="accent">External Accent</Link>
      </div>`,
    },
    {
      name: "Download",
      code: `
      <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        <Link href="/file.pdf" download>Download File</Link>
        <Link href="/file.pdf" download="custom-name.pdf">Download As</Link>
      </div>`,
    },
  ]}
/>

## Accessibility

The Link component follows accessibility best practices:

- Uses native `<a>` elements with proper `role="link"`
- External links include `target="_blank"` and `rel="noopener noreferrer"`
- External links have aria-labels indicating they open in new tabs
- Download links include appropriate aria-labels
- Maintains color contrast ratios for all variants
