...
- `infoTooltip`: Tooltip text to show next to the label (string)
...
InputField comes in two style variants: default (with border) and borderless (with shadow).
...
<div direction="column" gap="medium">
  ...
  <InputField 
    ...
    placeholder="With shadow instead of border"
    variant="borderless"
  />
</div>
`}
...
InputField with an information tooltip next to the label.
