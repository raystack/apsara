...
Apsara ships the 31 icons its own components draw. Each one is exported under a
...
| `UndoIcon` / `RedoIcon` | lucide `Undo2` / `Redo2` |
| `TextIcon` | lucide `Type` |
| `NumberedListIcon` / `ChecklistIcon` | lucide `ListOrdered` / `ListTodo` |
| `QuoteIcon` | lucide `TextQuote` |
| `CodeBlockIcon` | lucide `SquareCode` |
...
library that draws solid shapes ignores `stroke-width`.
...
|  | `@raystack/apsara/icons` | `@raystack/apsara` |
|---|---|---|
| Bundler honours `sideEffects` | one icon costs one icon | the same |
| Bundler ignores it | one icon costs one icon | pulls in every component |
| CommonJS `require` | loads the icons only | loads every component module |

The subpath reaches no component module at all, so it costs the same in every
case. The root re-exports every component alongside the icons and relies on
`"sideEffects": false` to have them stripped.

Prefer the subpath by default. Use the root in files that already import
