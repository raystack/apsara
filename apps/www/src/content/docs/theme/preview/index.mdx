...
Every setting is an ordinary prop, so React server-renders the seeded values on the first byte. What the server cannot know is what the browser has **stored**. The inline script closes that gap: it runs before first paint, reads the namespace, and patches every persisted, uncontrolled setting it finds there. A setting with nothing stored keeps the server-rendered value until React mounts, so on a first visit the default `appearance: "system"` paints light before the OS preference is applied.
