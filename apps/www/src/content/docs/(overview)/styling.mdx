...
Coverage note: `data-slot` is being rolled out component by component, starting with `Search`, `FilterChip`, and `DataView`. Each component's docs list its slot names as they land.
