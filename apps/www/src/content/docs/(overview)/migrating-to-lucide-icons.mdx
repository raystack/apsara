...
## 7. For anything Apsara does not ship, use `createIcon`

If you reach for an icon that is not one of the 243, wrap it yourself rather than
importing it raw — it then gets the same base props, the same `data-icon`, and the
same one-file swap as Apsara's own icons:
...
See [Icons](/docs/icons) for the naming rules, the override API, and the
gallery of the full set.
