...
- `CoPilotIcon` — unchanged. lucide has no equivalent, so it is still the
  in-house SVG.
- `FilterIcon` — same name, new drawing: lucide `ListFilter` in place of the
  in-house solid funnel.
...
See [Building your own icons](/docs/icons#building-your-own-icons).
...
Three keys are not in the map, because radix has no equivalent: `FilterIcon` and
`PanelLeftIcon` were in-house SVGs before, and `CoPilotIcon` still is.
...
This constraint applies to any runtime icon override, not just this design.

## Next

See [Icons](/docs/icons) for the naming rules, the override API, and the gallery
of the full set.
