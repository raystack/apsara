...
**Row keys:** For sortable or filterable tables, pass `getRowKey` so each row has a stable React key (e.g. `getRowKey={(row) => row.id}` or `getRowKey={(row) => row.uuid}`). This avoids rendering issues when rows reorder.
