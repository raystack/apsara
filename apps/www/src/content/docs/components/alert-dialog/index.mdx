...
An alert dialog cannot be dismissed by clicking away, so the work is deciding who owns its open state and what each of the two answers does.
...
A trigger and a content shell split into header, body and footer.
