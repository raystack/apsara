...
The block is assembled from parts, so a bare snippet and one with a header, a language switcher and a copy button all use the same root.
...
A root wrapping the code, plus the optional header furniture: a label, a language select, a copy button and a collapse trigger.
