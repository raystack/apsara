...
`Flex` is block-level by default, so it takes the full width of its parent. Set `inline` to render `display: inline-flex` instead: the container shrinks to its content and flows with the surrounding text, which suits a group of badges or an icon and label inside a sentence.
