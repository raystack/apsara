...
import { playground, inputDemo, autocompleteDemo, calendarPropsDemo, iconDemo, actionDemo, operationsDemo,
  controlledDemo,
} from "./demo.ts";
...
FilterChip supports five input types, `select`, `multiselect`, `date`, `string`, and `number`, to handle various filtering needs. `multiselect` takes a `string[]` value and summarizes two or more selections as "N selected". `number` restricts both typed and pasted input to numeric characters, so a non-numeric keystroke or a mixed paste such as `12ab` is rejected outright, and passes a `number` rather than a string to `onValueChange` once the value parses.
