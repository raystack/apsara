...
- **Grouping and selection don't mix yet.** With `group_by` active, group header rows enter the same row-id space as data rows, so ids can collide and a click may toggle the wrong rows. Hide the control (`<DataView.DisplayControls hideGrouping />`) while a selection column is present.
