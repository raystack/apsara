...
screen or opens at a position it is about to leave.
...
```tsx
<Tour steps={steps} defaultOpen transition="move" />
```
...
The root and its step shape, followed by the card parts to compose when the default card is not enough.
...
- The card is built on a Base UI `Popover`, so `Tour.Title` and `Tour.Description` provide
