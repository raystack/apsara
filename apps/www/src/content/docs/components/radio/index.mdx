---
title: Radio
description: A radio group component for selecting a single option from a list of options.
source: packages/raystack/components/radio
---

import { playground, stateDemo, sizeDemo, orientationDemo, labelDemo, formDemo,
  controlledDemo,
} from "./demo.ts";

<Demo data={playground} />

## Anatomy

Import and assemble the component:

```tsx
import { Radio } from "@raystack/apsara";

<Radio.Group>
  <Radio />
  <Radio />
</Radio.Group>
```

## Usage

Radios always live in a group. The group owns the value; the radios are the options.

### Size

Two sizes. `large` is the default; `small` fits dense lists. Set `size` on `Radio.Root` and every item follows.

<Demo data={sizeDemo} />

### Orientation

`vertical` (the default) stacks the options, which scans faster for more than three. `horizontal` suits short binary choices in a row.

<Demo data={orientationDemo} />

### With labels

Radio buttons should always be accompanied by labels for accessibility and usability.

<Demo data={labelDemo} />

### State

Radio buttons support different states to indicate interactivity and selection.

<Demo data={stateDemo} />

### Form example

Radio buttons can be used in forms with proper validation and submission handling.

<Demo data={formDemo} />

### Controlled

Pass `value` with `onValueChange` on `Radio.Group` to own the selection — needed when the choice drives something else on the page or is submitted with a form.

<Demo data={controlledDemo} />

## API Reference

The group that holds the value, and the radio itself.

### Group

Groups radio buttons and manages their shared state.

<auto-type-table path="./props.ts" name="RadioGroupProps" />

### Root

Renders an individual radio button.

<auto-type-table path="./props.ts" name="RadioProps" />

### Slots

Every rendered part carries a stable `data-slot` attribute for [styling and testing](/docs/styling#with-data-slot):

| Slot | Element |
|------|---------|
| `radio-group` | The `Radio.Group` container |
| `radio` | The radio button itself |
| `radio-indicator` | The indicator inside the radio button |

## Accessibility

- Follows the [WAI-ARIA Radio Group pattern](https://www.w3.org/WAI/ARIA/apg/patterns/radio/)
- Supports keyboard navigation with arrow keys within the group
- Uses `role="radiogroup"` for the group and `role="radio"` for items
- Selected state is communicated via `aria-checked`
