...
The root. Owns the selected value and the visible month, and provides both to every part.
...
The root renders no element of its own — it is a state owner, not a box — so `.Days` and `.Footer` stack on normal block flow but sit side by side inside a flex row. Give them a container when the surrounding layout is one.
