...
<Callout type="warn" title="Preview — not yet covered by semver">
  This component ships ahead of the rest of its family, and its API is still
  moving: RFC 005 plans a scale switcher and an input part for later phases,
  and both may reshape props, slots and `useCalendar()`'s return. Treat every
  name here as unstable until the family lands, and pin an exact version if
  you adopt it now. `Calendar` and `DatePicker` remain the supported choice.
</Callout>
