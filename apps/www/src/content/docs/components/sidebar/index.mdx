...
- `hideCollapsedItemTooltip` — whether item tooltips are disabled in the collapsed state
...
Use `collapseMode` to choose what the collapsed state looks like:

- `"icon"` (default): shrinks to an icon rail and pushes page content aside.
- `"hidden"`: disappears completely and reveals as a floating panel with a
  backdrop when opened. Clicking the backdrop or pressing Escape closes it.

For a responsive layout, drive it from your own breakpoint check:

```tsx
<Sidebar collapseMode={isMobile ? 'hidden' : 'icon'} />
```

When `collapseMode="hidden"` is closed, it shrinks to a thin strip just wide enough for the resize handle — reopen by clicking it, or place a `Sidebar.Trigger` as a direct child of `<Sidebar>` (not inside `Header`/`Main`/`Footer`, which are hidden along with everything else) for a clearer affordance, as in the "Hidden" tab below.

<Demo data={collapseModeDemo} />

*Note: the floating panel uses `position: fixed`, so it won't line up correctly if a parent element has its own `transform`, `filter`, or `contain` — avoid those on ancestors of the Sidebar when using `collapseMode="hidden"`.*

### Peek on hover

Set `peekOnHover` to temporarily reveal a collapsed sidebar when the user
hovers it, without changing the real `open` state — similar to VS Code's
auto-hide sidebar. Moving the mouse away reverts it; clicking the resize
handle or a `Sidebar.Trigger` pins it open for real. Clicking a `Sidebar.Item`
just navigates, like normal — it doesn't change the open state.
...
- Proper ARIA roles and attributes
  ...
  - `aria-expanded` to indicate sidebar state, on both the sidebar and `Sidebar.Trigger`
...
- Truncation handling
  ...
  - Items whose label is clipped show the full text in a tooltip on hover or focus

- Keyboard navigation support

  - Enter/Space to toggle sidebar expansion
  - Tab navigation through interactive elements
  - Escape closes the floating panel when `collapseMode="hidden"` is open
...
- Known limitations
  - The `collapseMode="hidden"` floating panel doesn't trap focus or set `aria-modal` — Tab can still reach content behind it. Close it (Escape, or the backdrop) before relying on background content being unreachable.
