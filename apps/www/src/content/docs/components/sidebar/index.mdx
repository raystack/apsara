...
Set `peekOnHover` to reveal a collapsed sidebar as an overlay while the user
hovers it, the way VS Code's auto-hide sidebar works. The overlay sits above the
content instead of pushing it, and the real `open` state never changes.

- **Moving the mouse away** reverts it.
- **The resize handle or a `Sidebar.Trigger`** pins it open for real, which does
  push content.
...
A root split into header, main and footer, with groups and items inside, plus `useSidebar` for reading the state from anywhere in the tree.

### Root

Groups all parts of the sidebar navigation. Use `collapsible` to choose what the collapsed state looks like (`"icon"` or `"hidden"`), or `"none"` to hide the resize handle and prevent toggling. `hideItemTooltips` disables the tooltips the Sidebar adds to items (collapsed labels, and clipped labels when expanded), and `peekOnHover` previews a collapsed sidebar on hover.
...
| `sidebar-toggle` | The resize handle that expands/collapses the sidebar (when `collapsible`) |
| `sidebar-trigger` | `Sidebar.Trigger` button |
| `sidebar-header` | `Sidebar.Header` container |
| `sidebar-main` | `Sidebar.Main` container |
| `sidebar-footer` | `Sidebar.Footer` container |
| `sidebar-group` | `Sidebar.Group` section |
| `sidebar-group-header` | Group header row |
| `sidebar-group-label` | Group label text |
| `sidebar-group-trigger` | Group header trigger (when `collapsible`) |
| `sidebar-group-chevron` | Group chevron icon (when `collapsible`) |
| `sidebar-group-panel` | Group items panel (when `collapsible`) |
| `sidebar-group-items` | Group items container |
| `sidebar-item` | `Sidebar.Item` link/element |
| `sidebar-item-text` | Item label text |
| `sidebar-leading-icon` | Leading icon/avatar shared by items, groups, and `Sidebar.More` |
| `sidebar-trailing-icon` | Trailing icon on a group header (when `trailingIcon` is set) |
| `sidebar-more-trigger` | `Sidebar.More` trigger row |
| `sidebar-more-text` | `Sidebar.More` trigger label |
