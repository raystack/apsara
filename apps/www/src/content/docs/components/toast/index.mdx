...
Toasts are dispatched from code rather than rendered inline. A provider holds them; a manager adds them.
...
Hook for dispatching toasts from inside a component tree wrapped by `<Toast.Provider>`. Returns the same methods as `createToastManager()`, including built-in `leadingIcon` support, plus a reactive `toasts` array reflecting the toasts on screen.
...
| Slot | Element |
|------|---------|
| `toast-viewport` | The region that holds all toasts |
| `toast` | A single toast root |
| `toast-content` | Content wrapper inside a toast |
| `toast-body` | Layout row holding the icon and text column |
| `toast-leading-icon` | The leading icon (unless `leadingIcon: null`) |
| `toast-main` | Column holding the title, actions, and description |
| `toast-header` | Row holding the title and action cluster |
| `toast-title` | The toast title |
| `toast-actions` | Cluster holding the action and close buttons |
