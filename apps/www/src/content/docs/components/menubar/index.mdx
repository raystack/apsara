...
A menubar is a row of menus sharing one keyboard focus, so most of its behaviour comes from [Menu](/docs/components/menu) itself.
...
One root. The menus inside it are plain `Menu`s.
