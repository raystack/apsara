...
import { preview, controlledDemo, boundedDragDemo, unreadBadgeDemo } from "./demo.ts";
...
- **`floating`** — the same element switched to `position: fixed`: drag it by
  ...
  and resize it from any edge or corner.
...
- Dragging clamps so the header can never leave the viewport (or the
  `dragBoundary` container), and the window re-clamps when the browser is
  resized.
