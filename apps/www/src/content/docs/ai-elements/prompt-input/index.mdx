...
The form wraps one of two inputs, `Textarea` or `Editor`, with the header, footer and submit around it.
...
Pass `inputRef` if you render your own input. To opt a region out, give it an
`onMouseDown` that calls `preventDefault()`.
...
A few `Textarea` props work differently here:

| On `Textarea` | On `Editor` |
|---|---|
| `readOnly` | `disabled` |
| `onChange` | `onValueChange`, on the root |
| `rows` | CSS `min-height` |
| `ref` to an `HTMLTextAreaElement` | `ref` to an `HTMLDivElement` |
