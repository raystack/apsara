...
describe('Separator', () => {
  ...
  describe('Combinations', () => {
    it('renders with all props combined', () => {
      ...
      const separator = screen.getByRole('separator');
      expect(separator).toHaveClass(styles['separator-half']);
      expect(separator).toHaveClass(styles['separator-secondary']);
    ...
    it.each(['horizontal', 'vertical'] as const)(
      'renders %s separator with all sizes',
      orientation => {
        const sizes = ['small', 'half', 'full'] as const;
        sizes.forEach(size => {
    ...
    it.each(['horizontal', 'vertical'] as const)(
      'renders %s separator with all colors',
      orientation => {
        const colors = ['primary', 'secondary', 'tertiary'] as const;
        colors.forEach(color => {
          const { container } = render(
            <Separator orientation={orientation} color={color} />
          );
          const separator = container.querySelector('[role="separator"]');
          expect(separator).toHaveClass(styles[`separator-${color}`]);
          expect(separator).toHaveAttribute('aria-orientation', orientation);
