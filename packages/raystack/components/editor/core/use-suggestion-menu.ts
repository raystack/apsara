...
/** The fields the menu reads from a row. */
 ...
 */
export function useMentionRegistryVersion(registry: MentionRegistry): void {
  useSyncExternalStore(
    registry.subscribe,
 ...
 */
export function toGroups<Item extends SuggestionItem>(
  items: Item[]
): SuggestionGroup<Item>[] {
  ...
  const groups: SuggestionGroup<Item>[] = [];
...
/** Sync data filtering: match-sorter on the accessors, best matches first. */
...
const byLabel = (item: SuggestionItem) => item.label;

function filterByLabel<Item extends SuggestionItem>(
  items: Item[],
  query: string
): Item[] {
  return filterItems(items, query, [byLabel]);
}

function firstEnabled(items: SuggestionItem[]): number {
...
export interface UseSuggestionMenuOptions<Item extends SuggestionItem> {
  getView: () => EditorView | null;
  /** The active query, or null. */
  suggestion: SuggestionState | null;
  /** Whether this menu owns the active query and may show. */
  open: boolean;
  /** Sync data, filtered with `filter`. */
  items?: Item[];
  /** Async data. Wins over `items`. */
  onSearch?: (
    query: string,
    context: { trigger: string; signal: AbortSignal }
  ) => Promise<Item[]>;
  /** Filters `items` for a query. Matches on the label by default. */
  filter?: (items: Item[], query: string) => Item[];
  onSelect: (item: Item, state: SuggestionState) => void;
  /** Closes the menu and leaves the typed text literal. */
  dismiss: () => void;
  onOpenChange?: (open: boolean) => void;
  listboxId: string;
  /** The part name in the dev warning for a rejected `onSearch`. */
  name: string;
}

export interface UseSuggestionMenuResult<Item extends SuggestionItem> {
  open: boolean;
  anchor: SuggestionAnchor;
  groups: SuggestionGroup<Item>[];
  /** The rows in display order. `highlightedIndex` indexes into it. */
  flat: Item[];
...
export function useSuggestionMenu<Item extends SuggestionItem>({
  getView,
  suggestion,
  open,
  items: syncItems,
  ...
  listboxId,
  name
}: UseSuggestionMenuOptions<Item>): UseSuggestionMenuResult<Item> {
  ...
  useEffect(() => {
    ...
    if (!search) {
      setLoading(false);
      setResults(filterRef.current(syncItems ?? [], query));
      return;
    }
      ...
      search(query, { trigger, signal: controller.signal })
        ...
        .catch((error: unknown) => {
          ...
          if (
            ...
            console.warn(
              `[Apsara] ${name} onSearch rejected; the menu falls ` +
  ...
  }, [open, trigger, query, search, syncItems, name]);
  ...
  if (onOpenChange) onOpenChangeRef.current = onOpenChange;
  ...
  const select = useCallback((item: Item) => {
    ...
    if (!state) return;
    onSelectRef.current(item, state);
  }, []);
