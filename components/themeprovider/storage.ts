...
export function isValidThemePreference(theme: string | null): theme is ThemePreference {
    return theme == "auto" || theme == "dark" || theme == "light";
}

export const saveThemePreference = (newTheme: ThemePreference) => {
    try {
        if (typeof newTheme === "string") window.localStorage.setItem(themeLocalStorageKey, newTheme);
    } catch (e) {
        console.warn(e);
    }
};
