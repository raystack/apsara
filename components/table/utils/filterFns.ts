...
const raypoint = <TData extends Record<string, any> = {}>(
    row: Row<TData>,
    id: string,
    _filterValue: any,
    addMeta: (item: RankingInfo) => void,
) => {
    if (typeof _filterValue === "string") return fuzzy(row, id, _filterValue, addMeta);

    const { filterFn, value } = _filterValue ?? {};
    switch (filterFn) {
        default: {
            return fuzzy(row, id, value, addMeta);
        }
    }
};

raypoint.autoRemove = (val: any) => !val;

export const RaypointFilterFns = {
    ...filterFns,
    between,
    betweenInclusive,
    contains,
